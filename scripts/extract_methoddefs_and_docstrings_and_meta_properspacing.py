...
def process_function(node, out_decl_fd, out_desc_fd, out_body_fd, out_meta_fd, input_filename, parent_class_lineno):
    ...
    if (pretty_docstring == "") or (processed_body_str == "") or (funcdef == ""):
        return
    print >> out_desc_fd, pretty_docstring
    print >> out_body_fd, processed_body_str
    print >> out_decl_fd, funcdef
    print >> out_meta_fd, meta_str

def process_class(node, out_decl_fd, out_desc_fd, out_body_fd, out_meta_fd, input_filename, parent_class_lineno):
    for inner_node in node.body:
        if isinstance(inner_node, ast.ClassDef):
            process_class(inner_node, out_decl_fd, out_desc_fd, out_body_fd, out_meta_fd, input_filename, node.lineno)
        elif isinstance(inner_node, ast.FunctionDef):
            process_function(inner_node, out_decl_fd, out_desc_fd, out_body_fd, out_meta_fd, input_filename, node.lineno)
    
    
def process_module(in_fd, out_decl_fd, out_desc_fd, out_body_fd, out_meta_fd, input_filename):
    module_str = in_fd.read()
    module_ast = ast.parse(module_str)
    for node in module_ast.body:
        if isinstance(node, ast.ClassDef):
            process_class(node, out_decl_fd, out_desc_fd, out_body_fd, out_meta_fd, input_filename, -1)

def usage():
    print >> sys.stderr, 'Usage:'
    print >> sys.stderr, sys.argv[0], 'out-declarations-file out-descriptions-file out-bodies-file out-metadata-file < in-files-list'
    sys.exit(-1)
...
def main():
    ...
    for line in sys.stdin:
        ...
        try:
            process_module(open(input_filename), funcdecl_fd, descriptions_fd, bodies_fd, meta_fd, input_filename)
        except SyntaxError:
