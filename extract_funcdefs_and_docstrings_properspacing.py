...
def process_function(node, out_decl_fd, out_desc_fd, out_body_fd):
...
def process_module(in_fd, out_decl_fd, out_desc_fd, out_body_fd):
    module_str = in_fd.read()
    module_ast = ast.parse(module_str)
    for node in module_ast.body:
        if isinstance(node, ast.FunctionDef):
            process_function(node, out_decl_fd, out_desc_fd, out_body_fd)

def usage():
    print >> sys.stderr, 'Usage:'
    print >> sys.stderr, sys.argv[0], 'out-declarations-file out-descriptions-file out-bodies-file < in-files-list'
    sys.exit(-1)
...
def main():
    ...
    for line in sys.stdin:
        ...
        try:
            process_module(open(input_filename), funcdecl_fd, descriptions_fd, bodies_fd)
        except SyntaxError:
